  ...
  <aside aria-label="Keranjang" class="flex h-full w-[372px] flex-none flex-col border-l border-line-2 bg-white">
    <div class="flex items-center justify-between gap-2.5 border-b border-line-3 px-[18px] pb-3 pt-[18px]">
      <div class="flex flex-col gap-0.5">
        <span class="text-[15px] font-bold tracking-tight">Keranjang</span>
        <span class="text-[11.5px] text-muted-3">{{ cart.count }} item</span>
      </div>
      <button
        type="button"
    ...
    <div class="flex-1 overflow-auto px-3 py-2">
      <EmptyState v-if="cart.isEmpty" icon="ph-shopping-cart-simple" message="Pilih produk untuk mulai transaksi" />
      <div v-for="item in cart.items" :key="item.variant_id" class="flex gap-2.5 border-b border-line-6 py-2.5 last:border-b-0">
        ...
        <div class="flex flex-col items-end gap-1.5">
          ...
          <div class="flex items-center gap-0.5 overflow-hidden rounded-lg border border-line">
            <button type="button" class="flex h-[30px] w-[30px] items-center justify-center text-muted-5 hover:bg-line-7" :aria-label="`Kurangi ${item.name}`" @click="cart.decrement(item.variant_id)">
    ...
    <div class="flex flex-col gap-3 border-t border-line-3 bg-surface-subtle px-[18px] pb-[18px] pt-3.5">
      ...
      <div class="flex items-center justify-between text-[13px]"><span class="text-muted">Subtotal</span><span class="font-semibold">{{ formatIDR(subtotalNum) }}</span></div>
      ...
      <BaseButton size="lg" class="w-full" :disabled="cart.isEmpty || !canCheckout" @click="emit('pay')">
        Bayar<i class="ph-duotone ph-arrow-right text-[18px]" aria-hidden="true"></i>
      </BaseButton>
